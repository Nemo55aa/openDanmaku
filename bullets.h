...
//  first made
// INFO OF VER 0.1:
//  fixed trueEachNframe (static int -> pointer)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// 
...
bool trueEachNframe(int timecnt, int n, int *prevCnt) {
	if (timecnt - *prevCnt == n) {
		*prevCnt = timecnt;
		return true;
	}
	else if ((timecnt - *prevCnt) > n) {		// prevCnt前回フレームカウント値リセット
		*prevCnt = timecnt;
		return false;
	}
	else {
...
/*void shootCometBullets(vector center, int range, int speed, int timeCnt, int duration, int hImg, obj blt[]) {
	if (trueEachNframe(timeCnt, duration, 32765) == true) {
...
void shootCometBullets(vector center, int range, int speed, int timeCnt, int duration, obj blt[]) {
	if (trueEachNframe(timeCnt, duration, 32764) == true) {
		vector iposSrc, iposDst;
		iposSrc.y = -80;
		//iposDst.y = HDHEIGHT + 80;
		iposDst.y = 1280 + 80;
