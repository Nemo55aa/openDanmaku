...
void readScript() {
	...
	er = fopen_s(&fp, SCRIPTPATH, "r");

	if (er != 0){
		MessageBox(NULL, TEXT("file does not Exist"), TEXT("note"), MB_OK);
	}
}
...
kaiwaClass::kaiwaClass(int bgImg, int bgmH, int c1img, int c2img) {
	kaiwaClass::backImg = bgImg;
	kaiwaClass::bgm = bgmH;
	kaiwaClass::chara1img = c1img;
	kaiwaClass::chara2img = c2img;
}

void kaiwaClass::chara1hide() {
	SetDrawBlendMode(DX_BLENDMODE_ALPHA, 128);
	DrawGraph(0, 0, chara1img, true);
	SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 255);
}
void kaiwaClass::chara2hide() {
	DrawGraph(0, 0, chara1img, true);
}
...
void danmakuScene(int* msg) {
	...
	p1.sethitsize({ 10, 10 });
	p1.setpos({ CENTER }); 
	p1.setMoveArea({ HDWIDTH - 100 * 2, HDHEIGHT - 100 * 2 }, { 100, 100 });
	bool defFlag = false;
	...
	while (ProcessMessage() == 0)
	{
		...
		if (ctoi(timeCnt) <= 8 && ctoi(timeCnt) % 3) {
			if (trueEachNframe(timeCnt, 9, 0) == true) {
				// N-way弾を発射
		...
		// 隕石的な弾幕１（緩め）
		if (ctoi(timeCnt) <= 10) {
			shootCometBullets({ CENTER }, 1000, rand() % 6 + 1, timeCnt, 9, redB, bullets);
		}
		// 隕石的な弾幕２（きつめ）
		if (ctoi(timeCnt) >= 11 && ctoi(timeCnt) <= 21) {
			shootCometBullets({ CENTER }, 1000, rand() % 8 + 3, timeCnt, 10, blueB, 32766, bullets);
			shootCometBullets({ CENTER }, 1000, rand() % 8 + 3, timeCnt, 10, blueB, 32765, bullets);
		}

		if (ctoi(timeCnt) >= 22 && ctoi(timeCnt) <= 27) {
			// 20フレーム毎に
			if (trueEachNframe(timeCnt, 20, 1) == true) {
				// N-way弾を発射
				shootAimingBullet(ePosTmp, pPosTmp, 3.4, bullets);
				shootNwayBullets(ePosTmp, { 3.4, M_PI / 2 }, 10, 9, bullets);
			}
			if (trueEachNframe(timeCnt, 30, 254) == true && timeCnt % 3) {
		...
		if (ctoi(timeCnt) > 28 && ctoi(timeCnt) <= 40) {
			// 20フレーム毎に
			if (trueEachNframe(timeCnt, 20, 2) == true) {
				// 狙い撃ち弾を発射
				shootAimingBullet({ ePosTmp.x + 100, 100 }, pPosTmp, 5, redB, bullets);
				shootAimingBullet({ ePosTmp.x - 100, 100 }, pPosTmp, 5, redB, bullets);
			}
			if (trueEachNframe(timeCnt, 10, 100) == true && ctoi(timeCnt) %2) {
				shootNwayBullets(ePosTmp, { 3.4, M_PI / 2 }, 10, 36, blueB, bullets);
			}
		}

		if (ctoi(timeCnt) >= 41) {
			if ((timeCnt - prevCntEnd) >= 2 * 0.016) {
				cometDuration++;
				prevCntEnd = ctoi(timeCnt);
			}
			
			shootCometBullets({ CENTER }, 1000, 10, timeCnt, 10, bullets);
		}
